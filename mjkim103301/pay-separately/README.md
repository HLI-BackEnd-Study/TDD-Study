...
- [X] 내가 정산해야 할 목록을 조회할 방법 생각하기
- [X] 정산금을 요청받은 사람 목록이 1명 이상인지 확인
- [X] 할인받은 금액은 Settlement에 표기
- [X] 컴포넌트로 분리하기. 어플리캐이션 로직, 비지니스 로직(컴포넌트로 이동하기) 분리하기
- [X] API 기능 완성하기
- [X] Kotlin-exposed 적용하기
- [X] 테스트코드 모두 exposed로 변경하기
- [ ] 동시성 이슈 해결 방법 고민하기 (ex. 분산락)
